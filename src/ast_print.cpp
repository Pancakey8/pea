...
struct ExprPrinter {
  ...
  void operator()(const BinaryOp& b) {
    ...
    os << ",\"left\":";
    if (b.left) os << *b.left; else os << "null";
    os << ",\"right\":";
    if (b.right) os << *b.right; else os << "null";
    os << "}";
  }
  ...
  void operator()(const UnaryOp& u) {
    ...
    os << ",\"operand\":";
    if (u.operand) os << *u.operand; else os << "null";
    os << "}";
  }

  void operator()(const Call& c) {
    os << "{\"type\":\"Call\",\"callee\":";
    if (c.callee) os << *c.callee; else os << "null";
    os << ",\"args\":[";
    for (size_t i = 0; i < c.args.size(); ++i) {
      if (c.args[i]) os << *c.args[i]; else os << "null";
      if (i < c.args.size() - 1) os << ",";
    }
    os << "]}";
  }
};
...
struct StmtPrinter {
  ...
  void operator()(const IfStmt& s) {
    ...
    if (s.condition) os << *s.condition; else os << "null";
    os << ",\"then_branch\":[";
    for (size_t i = 0; i < s.then_branch.size(); ++i) {
      if (s.then_branch[i]) os << *s.then_branch[i]; else os << "null";
      if (i < s.then_branch.size() - 1) os << ",";
    }
    os << "],\"else_branch\":";
    std::visit([this](auto&& branch) {
      ...
      if constexpr (std::is_same_v<T, std::monostate>) {
        ...
        for (size_t i = 0; i < branch.size(); ++i) {
          if (branch[i]) os << *branch[i]; else os << "null";
          if (i < branch.size() - 1) os << ",";
        }
        os << "]";
      } else if constexpr (std::is_same_v<T, StmtPtr>) {
        if (branch) os << *branch; else os << "null";
      }
    }, s.else_branch);
    os << "}";
  }
  ...
  void operator()(const ForStmt& s) {
    ...
    os << ",\"start\":";
    if (s.start) os << *s.start; else os << "null";
    os << ",\"end\":";
    if (s.end) os << *s.end; else os << "null";
    os << ",\"step\":";
    if (s.step) os << *s.step; else os << "null";
    os << ",\"body\":[";
  ...
  void operator()(const ExprStmt& s) {
    ...
    if (s.expr) os << *s.expr; else os << "null";
    os << "}";
  }
...
std::ostream& operator<<(std::ostream& os, const Program& prog) {
  ...
  for (size_t i = 0; i < prog.statements.size(); ++i) {
    if (prog.statements[i]) os << *prog.statements[i]; else os << "null";
    if (i < prog.statements.size() - 1) os << ",";
  }
  os << "]}";
  return os;
}
