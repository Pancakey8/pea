...
void Parser::advance() {
    ...
    else current = Token{TokenType::EndOfFile, ""};
}

std::expected<void, std::string> Parser::consume(TokenType type, std::string_view msg) {
    if (current.type == type) { advance(); return {}; }
    return std::unexpected(std::string(msg) + " at line " + std::to_string(current.line));
}

int Parser::get_precedence(TokenType type) {
    switch (type) {
        case TokenType::KW_Or: case TokenType::KW_And: return 2;
        case TokenType::Eq: case TokenType::Neq: case TokenType::Less: 
        case TokenType::Great: case TokenType::Lteq: case TokenType::Gteq: return 5;
        case TokenType::KW_Mod: case TokenType::Amp: return 10;
        case TokenType::Plus: case TokenType::Minus: return 20;
        case TokenType::Star: case TokenType::Slash: case TokenType::Backslash: return 30;
...
std::expected<ExprPtr, std::string> Parser::parse_prefix() {
    if (current.type == TokenType::Number) {
        auto val = current.number; advance();
        return std::make_unique<Expr>(Literal{val});
    }
    if (current.type == TokenType::String) {
        auto val = current.text; advance();
        return std::make_unique<Expr>(Literal{val});
    }
    if (current.type == TokenType::Char) {
        auto val = current.character; advance();
        return std::make_unique<Expr>(Literal{val});
    }
    if (current.type == TokenType::Ident) {
        auto name = current.text; advance();
...
std::expected<ExprPtr, std::string> Parser::parse_infix(ExprPtr left) {
    if (current.type == TokenType::LParen) {
        ...
        if (current.type != TokenType::RParen) {
            do {
                ...
                if (!arg) return std::unexpected("Invalid argument");
    ...
    return std::make_unique<Expr>(BinaryOp{op, std::move(left), std::move(*right)});
}
...
std::expected<StmtPtr, std::string> Parser::parse_statement() {
    ...
    if (current.type == TokenType::Ident) {
        ...
        // For simplicity in this implementation, we'll check if it's a keyword first.
    }
    ...
    if (current.type == TokenType::KW_Dim) return parse_dim();
    if (current.type == TokenType::KW_Let) return parse_let();
    if (current.type == TokenType::KW_If) return parse_if();
    if (current.type == TokenType::KW_For) return parse_for();
    if (current.type == TokenType::KW_Goto) return parse_goto();
    if (current.type == TokenType::KW_Do) return parse_do();

    // Expression statement
    auto expr = parse_expression();
    if (!expr) return expr;
    consume(TokenType::EOL, "Expected EOL");
    return std::make_unique<Stmt>(ExprStmt{std::move(*expr)});
}

std::expected<StmtPtr, std::string> Parser::parse_dim() {
    advance(); // dim
    std::string name = current.text; consume(TokenType::Ident, "Expected identifier");
    consume(TokenType::KW_As, "Expected 'as'");
    std::string type = current.text; consume(TokenType::Ident, "Expected type");
    ExprPtr init = nullptr;
    if (current.type == TokenType::Eq) {
        advance();
        auto e = parse_expression();
        if (!e) return std::unexpected("Invalid init expression");
        init = std::move(*e);
    }
    consume(TokenType::EOL, "Expected EOL");
...
std::expected<StmtPtr, std::string> Parser::parse_if() {
    ...
    if (!then_branch) return std::unexpected("Invalid then branch");
    ...
    while (current.type == TokenType::KW_Else) {
        ...
        if (current.type == TokenType::KW_If) {
            ...
            if (!nested_if) return std::unexpected("Invalid else if");
            else_branch.push_back(std::move(*nested_if));
        } else {
            auto stmts = parse_statements(TokenType::KW_EndIf);
            if (!stmts) return std::unexpected("Invalid else branch");
            for (auto& s : *stmts) else_branch.push_back(std::move(s));
        }
    }
    consume(TokenType::KW_EndIf, "Expected 'end if'");
    return std::make_unique<Stmt>(IfStmt{std::move(*cond), std::move(*then_branch), std::move(else_branch)});
}

std::expected<StmtPtr, std::string> Parser::parse_for() {
    advance(); // for
    std::string var = current.text; consume(TokenType::Ident, "Expected identifier");
    consume(TokenType::Eq, "Expected '='");
    auto start = parse_expression(); if (!start) return start;
    consume(TokenType::KW_To, "Expected 'to'");
    auto end = parse_expression(); if (!end) return end;
    ExprPtr step = nullptr;
    if (current.type == TokenType::KW_Step) {
        advance();
        step = parse_expression(); if (!step) return std::unexpected("Invalid step");
        step = std::move(*step);
    }
    consume(TokenType::EOL, "Expected EOL");
    auto body = parse_statements(TokenType::KW_EndFor);
    if (!body) return std::unexpected("Invalid for body");
    consume(TokenType::KW_EndFor, "Expected 'end for'");
    return std::make_unique<Stmt>(ForStmt{var, std::move(*start), std::move(*end), std::move(step), std::move(*body)});
}

std::expected<StmtPtr, std::string> Parser::parse_goto() {
    advance(); // goto
    std::string label = current.text; consume(TokenType::Ident, "Expected label");
    consume(TokenType::EOL, "Expected EOL");
...
std::expected<StmtPtr, std::string> Parser::parse_do() {
    ...
    if (while_at_start) {
        advance();
        cond = parse_expression(); if (!cond) return std::unexpected("Invalid while condition");
        cond = std::move(*cond);
        consume(TokenType::EOL, "Expected EOL");
    ...
    consume(TokenType::EOL, "Expected EOL");
    return std::make_unique<Stmt>(DoStmt{std::move(body), std::move(cond), while_at_start});
}

std::expected<std::vector<StmtPtr>, std::string> Parser::parse_statements(TokenType stop_token) {
...
std::expected<Program, std::string> Parser::parse() {
    ...
    if (!stmts) return std::unexpected(stmts.error());
