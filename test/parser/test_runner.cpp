...
bool run_test(const TestCase &tc) {
  ...
  if (tc.expected_err) {
    if (result) {
      std::println("FAIL: {}. Expected error, but parse succeeded.",
        tc.pea_path.filename().string());
      return false;
    }
  ...
  if (!tc.expected_json) {
    std::println("FAIL: {}. No expected output file provided.",
      tc.pea_path.filename().string());
    return false;
  }

  if (!result) {
    std::println("FAIL: {}. Parse failed: {}:{} to {}:{}: {}",
      tc.pea_path.filename().string(),
      result.error().range.start.line,
      result.error().range.start.col,
      result.error().range.end.line,
      result.error().range.end.col,
      result.error().message);
    return false;
  }
  ...
  for (const auto &entry : fs::directory_iterator(cases_dir)) {
    if (entry.path().extension() == ".pea") {
      ...
      if (fs::exists(json_path))
        tc.expected_json = json_path;
      if (fs::exists(err_path))
        tc.expected_err = err_path;
      tests.push_back(tc);
    }
  }
